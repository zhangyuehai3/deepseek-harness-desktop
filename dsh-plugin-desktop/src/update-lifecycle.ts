...
class DesktopUpdateLifecycleOwner implements DesktopUpdateLifecycle {
  ...
  private observeResult(result: UpdateCheckResult | null): string | undefined {
    ...
    this.availableForceUpdate = newer && result.forceUpdate
    this.availableUrl = newer ? downloadUrlForPlatform(result.urls, this.options.platform) : undefined
    ...
    const task = (async () => {
      ...
      try {
        confirmed = await this.options.adapter.confirmDownload(version, {
          ...
          ...(this.availableUrl === undefined ? {} : { downloadUrl: this.availableUrl }),
        })
      } catch {
        return
      }
      ...
      this.registration.refresh()
      try {
        await this.options.adapter.downloadAndOpen(version, controller.signal, this.availableUrl)
      } catch {
  ...
  private async offerDownload(version: string, automatic: boolean): Promise<void> {
    ...
    await this.stateReady
    if (this.disposed || (automatic && !this.availableForceUpdate && this.state.lastNotifiedVersion === version)) return
    // Automatic background checks only prompt when the service forces the update.
    // Manual tray checks still ask the user before downloading.
    if (automatic && !this.availableForceUpdate) return
  ...
  private async runBackgroundCheck(): Promise<void> {
    ...
    try {
      ...
      if (this.availableForceUpdate) {
        await this.offerDownload(version, true)
      } else {
