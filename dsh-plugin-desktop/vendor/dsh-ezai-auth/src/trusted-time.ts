...
let baseServerEpoch: number | null = null
let baseMonotonic: number = performance.now()

/**
 * Updates the trusted server time base using an HTTP `Date` header,
 * explicit timestamp, or Date instance.
 */
export function updateServerTime(headerOrTimestamp?: string | number | Date | null): void {
  if (headerOrTimestamp === undefined || headerOrTimestamp === null) return
 ...
 */
export function getTrustedDate(persistedLastActiveTime: number = 0): Date {
  let epoch: number
 ...
 */
export function resetTrustedTimeForTesting(): void {
