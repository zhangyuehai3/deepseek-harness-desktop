...
function installModelLockObserver(ctx) {
  ...
  const cleanUI2 = () => {
    ...
    for (const badge of badges) {
      ...
      if (text !== "\u7248\u672C 2.1.1" && (text === "\u9884\u89C8\u7248" || text === "Preview" || text && /^版本\s*2\./.test(text))) {
        badge.textContent = "\u7248\u672C 2.1.1";
      }
    }
  };
...
function apply(ctx) {
  ...
  ctx.inject(["commandUi", "remote", "remote.commands"], (scope) => {
    const rawCommandUi = scope.get("commandUi")?.[Symbol.for("cordis.original")] || scope.get("commandUi");
    if (!rawCommandUi) return;
    const origCandidates = rawCommandUi.candidates;
    if (origCandidates) {
      rawCommandUi.candidates = async function(...args) {
    ...
    if (origRegister) {
      rawCommandUi.register = function(contribution) {
    ...
    if (origDispatch) {
      rawCommandUi.dispatch = function(pick) {
