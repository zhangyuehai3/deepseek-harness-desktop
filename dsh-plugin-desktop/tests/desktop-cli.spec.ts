...
describe('packaged dsh bootstrap', () => {
  ...
  it('clears Node mode and dispatches the imported CLI exactly once', async () => {
    ...
    const argv = ['/Applications/EZAI Desktop', '/app.asar/lib/desktop-cli.js', '--dump-config']
    const runCli = vi.fn(async () => {})
    const load = vi.fn(async (url: string) => {
      expect(environment).toEqual({ KEEP: 'value' })
      expect(argv).toEqual([
        '/Applications/EZAI Desktop',
  ...
  it('uses the physical unpacked dependency tree only inside an Electron package', () => {
    expect(unpackedAsarPath('/Applications/EZAI Desktop.app/Contents/Resources/app.asar/node_modules/pkg'))
      .toBe('/Applications/EZAI Desktop.app/Contents/Resources/app.asar.unpacked/node_modules/pkg')
    expect(unpackedAsarPath('C:\\Program Files\\EZAI Desktop\\resources\\app.asar\\node_modules\\pkg'))
      .toBe('C:\\Program Files\\EZAI Desktop\\resources\\app.asar.unpacked\\node_modules\\pkg')
    expect(unpackedAsarPath('/Applications/EZAI Desktop.app/Contents/Resources/app.asar/package.json'))
      .toBe('/Applications/EZAI Desktop.app/Contents/Resources/app.asar.unpacked/package.json')
