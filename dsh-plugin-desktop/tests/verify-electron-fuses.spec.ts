...
describe('final Electron fuse verification', () => {
  it('maps only requested platform and architecture keys to complete runtime contexts', () => {
    const expected = [
      ...
      join('/build', 'mac-universal', 'EZAI Desktop.app', 'Contents', 'MacOS', 'EZAI Desktop'),
      join('/build', 'win-unpacked', 'EZAI Desktop.exe'),
      join('/build', 'win-arm64-unpacked', 'EZAI Desktop.exe'),
    ].sort()
  ...
  it('ignores a stale sibling architecture from an earlier build', () => {
    const expected = join('/build', 'win-unpacked', 'EZAI Desktop.exe')
    const stale = join('/build', 'win-arm64-unpacked', 'EZAI Desktop.exe')
  ...
  it('fails when one requested architecture is missing even if a sibling exists', () => {
    const x64Executable = join('/build', 'win-unpacked', 'EZAI Desktop.exe')

    expect(() => resolveFinalPackagedRuntimeContexts(
      result([{ key: 'win', archs: [Arch.x64, Arch.arm64] }]),
      filename => filename === x64Executable,
    )).toThrow(`win/arm64 at ${join('/build', 'win-arm64-unpacked', 'EZAI Desktop.exe')}`)
  })
  ...
  it('resolves a real target-name map through the target archs retained by NSIS', () => {
    ...
    const built = {
      outDir: '/build',
      configuration: { productName: 'EZAI Desktop' },
  ...
  it('checks every requested final executable after all artifact builds', async () => {
    const executables = [
      join('/build', 'mac-arm64', 'EZAI Desktop.app', 'Contents', 'MacOS', 'EZAI Desktop'),
      join('/build', 'mac', 'EZAI Desktop.app', 'Contents', 'MacOS', 'EZAI Desktop'),
    ]
    const events: string[] = []
  ...
  it.each([
    ...
    await expect(verifyElectronExecutableFuses('/build/EZAI Desktop.exe', read))
  ...
  it('wraps an unreadable final executable with its resolved path', async () => {
    ...
    await expect(verifyElectronExecutableFuses('/build/EZAI Desktop.exe', read))
      .rejects.toThrow('/build/EZAI Desktop.exe')
  })
})
