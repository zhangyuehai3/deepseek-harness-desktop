...
describe('desktop Host plugin', () => {
  ...
  it('serves the macOS native picker through a same-origin desktop route', async () => {
    const harness = createHarness('darwin')
    harness.pickDirectory.mockResolvedValue('/Users/test/workspace')
    apply(harness.ctx, config)
    const route = harness.route(DESKTOP_DIRECTORY_PICKER_PATH)
    expect(route).toEqual(expect.objectContaining({
      kind: 'exact',
    ...
    expect(harness.pickDirectory).toHaveBeenCalledOnce()
    expect(res.statusCode).toBe(200)
    expect(JSON.parse(body)).toEqual({ path: '/Users/test/workspace' })
  })
