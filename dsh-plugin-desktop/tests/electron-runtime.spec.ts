...
describe('Electron desktop runtime', () => {
  ...
  it('rejects when the Windows installer download fails', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('win32')
    updater.download.mockRejectedValueOnce(new Error('blocked'))
    const requestQuit = vi.fn()
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    runtime.schedule({ ...spec, requestQuit })
    ...
    expect(updater.record).not.toHaveBeenCalled()
    expect(childProcess.spawn).not.toHaveBeenCalled()
    expect(requestQuit).not.toHaveBeenCalled()
  })
  ...
  it('does not show macOS completion after the update download is aborted', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('darwin')
    updater.download.mockRejectedValueOnce(new DOMException('aborted', 'AbortError'))
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
