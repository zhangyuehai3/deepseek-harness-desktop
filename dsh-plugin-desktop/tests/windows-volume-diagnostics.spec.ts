...
describe('Windows volume diagnostics', () => {
  ...
  it('skips non-Windows hosts without touching the query', () => {
    const result = diagnoseWindowsVolumes('darwin', [{ label: 'EZAI home', path: '/Users/a/.dsh' }], () => {
  ...
  it('accepts fixed NTFS and ReFS volumes', () => {
    expect(diagnoseWindowsVolumes('win32', [{ label: 'install', path: 'C:\\App\\EZAI Desktop.exe' }], query({
      root: 'C:\\',
      fileSystem: 'NTFS',
      driveType: 3,
    }))).toEqual([])
    expect(diagnoseWindowsVolumes('win32', [{ label: 'install', path: 'D:\\App\\EZAI Desktop.exe' }], query({
      root: 'D:\\',
      fileSystem: 'REFS',
      driveType: 3,
    }))).toEqual([])
  ...
  it('formats the concern for stderr diagnostics', () => {
    expect(formatWindowsVolumeConcern({
      ...
      driveType: 2,
      reason: 'unsupported',
    })).toContain('EZAI home: unsupported (path=E:\\.dsh; root=E:\\; fs=FAT32; driveType=2)')
  })
})
