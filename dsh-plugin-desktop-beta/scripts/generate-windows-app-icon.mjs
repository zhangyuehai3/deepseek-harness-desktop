...
/**
 * Reuse the repository's EZAI mark for frames where the full shaded artwork
 * loses recognizable detail. The flat light rounded-square treatment keeps the
...
async function loadSmallFrameArtwork() {
  ...
  if (!source.includes(BRAND_ARTWORK) || /<style\b/iu.test(source)) {
    throw new Error('generate-windows-app-icon: tray-icon.svg must embed the repository-owned EZAI artwork')
  }
  const mark = source
    .replace(/^<svg[^>]*>\s*/u, '')
    .replace(/<\/svg>\s*$/u, '')
